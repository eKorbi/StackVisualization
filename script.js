...
class Stack extends Array {
	...
	//add the  new elements to the beginning of the stack.
	...
	shift_element() {
		...
		if (this.length === 0) {
			stack.innerHTML = 'Stack Is Empty, you can push elemets again 😎';
		} else if(this.length > 0) {
			let element = list.childNodes[0]; //initialize 'element' with the first child->first element of stack
			super.shift(); //remove the first element of the array
			list.removeChild(element); //remove the first element from list, the element's child (remove it from DOM)
			return element;
		}
	}
...
function Push() {
	let input = text.value; //get the input from the user and store it to input
	if (input == '' ) { //check if the user attempts to insert an empty value
	...
	//check if the stack is full -> allowed the stack to only have 10 elements 
		...
		s1.unshift_element(input); //call the unshift method (which inserts elements at the beginning)
		let y = document.createElement('li'); //create a <li></li> element
		let u = document.createTextNode(input); //create the text node
